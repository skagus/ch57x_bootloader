...
void USB_DevTransProcess(void)
{
	...
	HAL_DbgLog("IF: %8X, IS:%8X\n", R8_USB_INT_FG, R8_USB_INT_ST);
			...
			switch(R8_USB_INT_ST & (MASK_UIS_TOKEN | MASK_UIS_ENDP))
			{
				case UIS_TOKEN_IN | 0:	// D2H
				{
					switch(SetupReqCode)
					{
						...
						default:
							//The status stage completes the interrupt 
							// or forces the upload of a 0-length data packet to end the control transmission
							R8_UEP0_T_LEN = 0;
							R8_UEP0_CTRL = UEP_R_RES_ACK | UEP_T_RES_NAK;
							break;
					}
				...
				case UIS_TOKEN_OUT | 0:	// H2D
				{
					len = R8_USB_RX_LEN;
					if(SetupReqCode == USB_SET_CONFIGURATION)
					{
						if(pEP0_DataBuf[0])
...
/*********************************************************************
 * @fn      DevEP2_OUT_Deal
 * @brief   Handle Rcv on EP2, User defined.
 * @return  none
 */
